...
class EShoppingCart extends CMap {
    ...
    * @return bool
    */
    public function put(IECartPosition $position, $quantity = 1, $k = 0) {
	    $key = $position->getId();
	    if (is_array($key)) {
		    $key = $key[$k];
	    }
	    ...
	    if($this->update($position, $quantity, $k)){
		    return true;
	    }
    }
     ...
     * @param int $quantity
     */
    public function update(IECartPosition $position, $quantity, $k=0) {
	    ...
	    $position->setQuantity($quantity, $key);


	    if ($position->getQuantity($key) < 1)
		    $this->remove($key);
	    else
