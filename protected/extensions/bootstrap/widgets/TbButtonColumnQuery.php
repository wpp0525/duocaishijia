<?php
	 ...
	 */
	class TbButtonColumnQuery extends CButtonColumn
	{
		/**
		...
		protected function initDefaultButtons()
		{
			...
			if ($this->updateButtonIcon !== false && !isset($this->buttons['update']['icon'])) {
				...
				$this->buttons['update']['visible'] = !Yii::app()->user->isCustomService();  //不是客服才可见
			}
			if ($this->deleteButtonIcon !== false && !isset($this->buttons['delete']['icon'])) {   //更改和删除功能按钮隐藏掉。
				$this->buttons['delete']['icon'] = $this->deleteButtonIcon;
				$this->buttons['delete']['visible'] =  !Yii::app()->user->isCustomService();
			}
		}
		...
		{
			if (isset($button['visible']) && $button['visible'] == false)       //把更新和删除的按钮，隐藏起来
			{
				return;
			}
