...
//	            $this->handleSaveProp($model);
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($_POST['ItemProp'])) {  //如果没有添加最下面的属性，也会执行
            $this->handlePostData();
            $model->attributes = $_POST['ItemProp'];
	        $model->prop_desc = $_POST['ItemProp']['prop_desc'];


            if ($model->save()) {
	            $this->handleSaveProp($_POST['ItemProp']['propValues']);
                $this->redirect(array('view', 'id' => $model->item_prop_id));
            }
        }
            ...
            if (is_array($propValues['value_name']) && $count = count($propValues['value_name'])) {
                for ($i = 0; $i < $count; $i++) {
                    $_POST['ItemProp']['propValues'][] = array(
                        ...
                        'status' => 1,
	                    'pic'       =>  ($propValues['pic'][$i]==''? null:$propValues['pic'][$i]),
	                    'erpID'       =>  ($propValues['erpID'][$i]==''? null:$propValues['erpID'][$i]),
                    );
                }
            }
        }
	...
	protected function handleSaveProp($propvalues){
			foreach($propvalues as $propvalue){
				$sql = 'UPDATE prop_value SET erpID = "'.$propvalue['erpID'].'" WHERE value_name = "'.$propvalue['value_name'].'" ;';
