...
class ItemController extends MallBaseController
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Item'])) {
	        ...
	        $propvales = $_POST['Item']['propValues'];
            if ($model->save()) {                    //如果是物料增加，则直接插入，如果是更新则需要把更新的修改一下。
	            if(isset($propvales)){
		        foreach($propvales as $key => $propvale){
		            $itemProp = ItemProp::model()->findByPk($key);
		            if($itemProp->is_color_prop==1){
			            foreach($propvale as $propvalue){
				            $propvalue = PropValue::model()->findByPk($propvalue);
				            if(isset($propvalue)){
					            $imgPro = new PropImg();
					            $imgPro->attributes = $propvalue;
					            $imgPro->item_id = $model->item_id;
					            $imgPro->item_prop_value = $propvalue->value_name;
					            $imgPro->prop_value_id = $propvalue->prop_value_id;
					            $imgPro->pic = $propvalue->pic;
					            $imgPro->create_time = time();
					            if(!$imgPro->save()){
						            throw new Exception;
					            }
				            }
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($_POST['Item'])) {
            ...
            if ($model->save()) {  //如果是产品更新,则需要先删除信息，在插入到prop_img。 如果跑出异常，是价格为空，价格是必填的
	            $item_id_pk = $model->item_id;
				$propsimag = PropImg::model()->findAllByAttributes(array(
				   'item_id' => $model->item_id,
				));
			if(isset($propsimag)){
				PropImg::model()->deleteAll("item_id = :item_id_pk",array(":item_id_pk"=>$item_id_pk));//先把原来的老数据删除掉，在更新。
			}
...
//            $arr['stock'] = $sku->stock;
    ...
    protected function handlePostData()
    {
        ...
        if (isset($_POST['Item']['skus']['checkbox']) && is_array($_POST['Item']['skus']['checkbox'])) {
	        $propValues = $_POST['Item']['skus']['checkbox'];
	        $_POST['Item']['propValues'] = $propValues;
