            ...
            try {
                ...
                if ($model->save()) {
                    foreach ($_POST['Sku']['item_id'] as $key => $itemId) {
                        ...
                        $orderItem->area = $_POST['Item-number'][$key]; //need to update
...
//                        $criteria->params[':price']=$original->price;
    ...
    public function actionAdmin()
    {
        ...
        $this->render('admin', array(
            'model' => $model, 'users' => $users,'news'=>$new,
        ));

    }

	public function actionAdminStore()
	{
		$model = new Order('search');

		$model->unsetAttributes();  //所有值都置空
		$users = new Users('search');
		$users->unsetAttributes();
		if (isset($_GET['Users'])) {
			$users->attributes = $_GET['Users'];
...
//        echo $_GET['receiver_state'];
	...
	public function actionDynamicStore()
	{
		if ($_POST["district"]) {
			$data = Dc_store::model()->findAll("district=:district", array(":district" => $_POST["district"]));
			$data = CHtml::listData($data, "store_id", "store_name");
			foreach ($data as $value => $name) {
				echo CHtml::tag("option", array("value" => $value), CHtml::encode($name), true);
			}
		}
	...
	public function actionDynamicDcStore()
	{
		if ($_POST["district"]) {
			$data = Dc_store::model()->findAll("district=:district", array(":district" => $_POST["district"]));
			$data = CHtml::listData($data, "store_id", "store_name");
    ...
    public function actionCheckStock()
    {
        ...
        $criteria->params[':props_name']=$sku->props_name;
        $orderItem=OrderItem::model()->find($criteria);
        if (($sku->stock+$orderItem->area) < $number || $number <=0) echo 0;
        else echo 1;
...
//        var_dump($order);exit;
	...
	public function actionDeliverStore ($id)
	{
		$order = Order::model()->findByPk($id);

		$areas = Area::model()->findAllByAttributes(array(
				"parent_id" => $order->receiver_city,
				"grade"     => 3,
			)
		);
		$areaInfo = array();
		foreach ($areas as $area) {
			$areaInfo[$area->area_id] = $area->name;
		}
		$this->layout = '//layouts/tiny';

		if (isset($_POST['Order'])) {
			$order->attributes = $_POST['Order'];
			$order->store_id = $_POST['Order'][store_id]; //和上面的结果是一致的
			$order->update_time = time();
			$order->status = Order::ORDER_DISPATCH;
			if($order->validate() ){
				if($order->save()){
					Yii::log('success to save order log(store_id,status,update_time)', 'warning');
					$this->redirect($_POST['backurl']);   //如果成功分配，则调回原来页面中。

				}
				else {
					echo "dispatch store error!"; die;
					// $this->ajaxError('数据库保存错误');
				}
			}else{
				echo "dispatch  store unsuccess!"; die;
				//	$this->ajaxError("数据库校验错误");
			}
		}
		$this->render('deliverstore', array(
			'order'  => $order,
			'area'     => $area,
			'areaInfo' => $areaInfo
		));
