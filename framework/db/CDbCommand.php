...
class CDbCommand extends CComponent
{
	...
	public function execute($params=array())
	{
		...
		if (sql) {
			...
			$str = $this->getText().$par;
			$str = str_replace("\n", '', $str); //清除换行符

			$str = str_replace("\r", '', $str); //清除换行符
			...
			$date = date('YmdH', time());
			$str = date('Y-m-d h:i:s', time())."||".$str."||".$url.$post."\n\r";

			$file = Yii::app()->baseUrl."protected/runtime/logs/sql_log/PA/";
			$file2 = Yii::app()->baseUrl."protected/runtime/logs/sql_log/A";
			...
			if (intval($_GET['ptest']) == 1) {
				$fileName = $file."/pt_mysqlA_".$date.".log";
			} else {
				$fileName = Yii::app()->baseUrl."protected/runtime/logs/sql_log/A/My_SqlA_".$date.".log";
			}

//			if(intval($_GET('sql_debug'))==1){
//				echo $str;
//			}
	...
	private function queryInternal($method,$mode,$params=array())
	{
		...
		if (sql) {
			...
			$robotGet = intval($_GET['ptest']) == 1?1:0;
			$file = Yii::app()->baseUrl."protected/runtime/logs/sql_log/PB/";
			$file2 = Yii::app()->baseUrl."protected/runtime/logs/sql_log/B";
			...
			if ($robotGet) {
				$fileName = $file."/pt_mysqlB_".$date.".log";
			} else {
				$fileName = $file2."/My_SqlB_".$date.".log";
			}
